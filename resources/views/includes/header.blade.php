            ...
            <ul class="nav navbar-nav">
              ...
              <li class="dropdown userbtn">
                ...
                <ul class="dropdown-menu">
                  <li><a href="{{route('donor.home')}}"><i class="fa fa-tachometer" aria-hidden="true"></i> {{__('Dashboard')}}</a> </li>
                  <li><a href="{{ route('my.profile') }}"><i class="fa fa-user" aria-hidden="true"></i> {{__('My Profile')}}</a> </li>
                  <li><a href="{{ route('my.profile', Auth::user()->id) }}"><i class="fa fa-eye" aria-hidden="true"></i> {{__('View Public Profile')}}</a> </li>
              ...
              @endif @if(Auth::guard('charity')->check())
              <li class="postjob"><a href="{{route('post.campaign')}}">{{__('Post a job')}}</a> </li>
              <li class="dropdown userbtn"><a href="">{{Auth::guard('charity')->user()->printCharityImage()}}</a>
                <ul class="dropdown-menu">
                  <li><a href="{{route('charity.home')}}"><i class="fa fa-tachometer" aria-hidden="true"></i> {{__('Dashboard')}}</a> </li>
                  ...
                  <li><a href="{{ route('charity.logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form-header1').submit();">{{__('Logout')}}</a> </li>
                  <form id="logout-form-header1" action="{{ route('charity.logout') }}" method="POST" style="display: none;">
                    {{ csrf_field() }}
                  </form>
                </ul>
              </li>
              @endif @if(!Auth::user() && !Auth::guard('charity')->user())
