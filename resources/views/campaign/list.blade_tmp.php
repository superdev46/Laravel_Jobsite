...
<div class="listpgWraper">
  ...
  @include('flash::message')
    ...
    <form action="{{route('campaign.list')}}" method="get">
      ...
      <div class="pageSearch">
        ...
        <div class="row">
          ...
          <a href="{{ route('post.campaign') }}" class="btn"><i class="fa fa-file-text" aria-hidden="true"></i> {{__('Post Job')}}</a>
    ...
    <form action="{{route('job.list')}}" method="get">
      ...
      <div class="row"> @include('includes.campaign_list_side_bar')
            ...
            <li>
              ...
              <div class="row">
                ...
                <div class="col-md-8 col-sm-8">
                  ...
                  <div class="jobimg">{{$charity->printCampaginImage()}}</div>

                  <div class="jobinfo">

                    <h3><a href="{{route('campaign.detail', [$campaign->slug])}}" title="{{$campaign->title}}">{{$campaign->title}}</a></h3>

                    <div class="companyName"><a href="{{route('charity.detail', $charity->slug)}}" title="{{$charity->name}}">{{$charity->name}}</a></div>

                    <div class="location">
					            <label class="fulltime" title="{{$campaign->getJobType('job_type')}}">{{$campaign->getJobType('job_type')}}</label>
                      - <span>{{$campaign->getCity('city')}}</span>
                    </div>
                  </div>
                ...
                <div class="col-md-4 col-sm-4">
                  ...
                  <div class="listbtn"><a href="{{route('campaign.detail', [$campaign->slug])}}">{{__('View Details')}}</a></div>
                </div>
              </div>
              <p>{{str_limit(strip_tags($campaign->description), 150, '...')}}</p>
            </li>
          ...
          <div class="pagiWrap">
            ...
            <div class="row">
              ...
              <div class="col-md-5">
                ...
                {{__('Showing Pages')}} : {{ $campaigns->firstItem() }} - {{ $campaigns->lastItem() }} {{__('Total')}} {{ $campaigns->total() }}
