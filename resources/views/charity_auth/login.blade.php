		...
		<div class="col-xl-5 offset-xl-3">
			...
			<div class="login-register-page">
				...
				<div class="account-type">
					<div>
						<a class="button full-width btn-account-login"><i class="icon-material-outline-business-center"></i> Charity</a>
					</div>

					<div>

						<a href="/donor/login" class="button full-width btn-account-login active"><i class="icon-material-outline-account-circle"></i> Donor</a>
					</div>
        		</div>
					
				<!-- Form -->
        <form method="post" id="login-form" action="{{ route('charity.login') }}">
          {{ csrf_field() }}
					<div class="input-with-icon-left{{ $errors->has('email') ? ' has-error' : '' }}">
						<i class="icon-material-baseline-mail-outline"></i>
						<input type="text" class="input-text with-border" name="email" id="emailaddress" placeholder="Email Address" value="" required/>
            @if ($errors->has('email'))
