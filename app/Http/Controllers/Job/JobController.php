...
class JobController extends Controller
{
    ...
    {
		$this->middleware('auth', ['except' => ['jobsBySearch', 'jobDetail', 'projectsBySearch', 'postApplyProject']]);
	...
	public function projectsBySearch(Request $request)
    {

        $search = $request->query('search', '');
		$job_titles = $request->query('job_title', array());
		$donor_ids = $request->query('donor_id', array());
		$industry_ids = $request->query('industry_id', array());
		$job_skill_ids = $request->query('job_skill_id', array());
		$functional_area_ids = $request->query('functional_area_id', array());
		$country_ids = $request->query('country_id', array());
        ...
        $jobs = $this->fetchJobs($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids, $functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, $order_by, $limit);
		
		/*****************************************************/
		
		$jobTitlesArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.title');
		
		/****************************************************/
		
		$jobIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.id');
		...
		$countryIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.country_id');
		
		/*****************************************************/
		
		$stateIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.state_id');
		
		/*****************************************************/
		
		$cityIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.city_id');
		
		/*****************************************************/
		
		$donorIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.donor_id');
		
		/*****************************************************/
		
		$industryIdsArray = $this->fetchIndustryIdsArray($donorIdsArray);
		
		/*****************************************************/
		
		$functionalAreaIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.functional_area_id');
		
		
		/*****************************************************/
		
		$jobTypeIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.job_type_id');
		
		/*****************************************************/
		
		$jobShiftIdsArray = $this->fetchIdsArray($search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, 'jobs.job_shift_id');
	
		/*****************************************************/
		
		$seoArray = $this->getSEO($functional_area_ids, $country_ids, $state_ids, $city_ids);
        ...
        return view('job.projectlist')
						...
						->with('stateIdsArray', $stateIdsArray)
						->with('cityIdsArray', $cityIdsArray)						
						->with('donorIdsArray', $donorIdsArray)
						->with('industryIdsArray', $industryIdsArray)
						->with('functionalAreaIdsArray',$functionalAreaIdsArray)
						->with('jobTypeIdsArray',$jobTypeIdsArray)
						->with('jobShiftIdsArray',$jobShiftIdsArray)
	...
	{		
			...
			$applied_state = 0;
			$jobApplies = $job->jobApplies()->get();	
			$current_charity = Auth::guard('charity')->user();
			foreach($jobApplies as $apply)
			{
				if($apply->charity_id == $current_charity->id)
				{
					$applied_state = 1;
					break;
				}
			...
			$relatedJobs = $this->fetchJobs($search, $job_titles, $company_ids, $industry_ids, $job_skill_ids, $functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured, $order_by, $limit);
			/********************************************/
			...
			return view('job.projectdetail')
                        ->with('job', $job)
						->with('relatedJobs', $relatedJobs)
						->with('seo', $seo)
						->with('applies', $jobApplies)
						->with('applied_state', $applied_state);
	}
	...
	public function postApplyProject(ApplyJobFormRequest $request, $job_slug)
	{
		...
		$jobApply->charity_id = $charity_id;
		$jobApply->job_id = $job->id;
		$jobApply->description = $request->post('description');
		$jobApply->current_salary = $request->post('current_salary');
		$jobApply->period = $request->post('period');
		$jobApply->save();
