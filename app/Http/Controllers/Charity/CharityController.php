...
use App\Http\Requests\Front\CharityFrontFormRequest;
use App\Http\Controllers\Controller;
use App\Traits\CharityTrait;
use App\Traits\FetchDonors;
...
class CharityController extends Controller
{
    ...
    {
        $this->middleware('charity', ['except' => ['companyDetail', 'sendContactForm']]);
    }

    public function index()
    {
        return view('charity_home');
    }
	...
	public function charityProfile()
    {
        ...
        // dd($charity);
        return view('charity.edit_profile')
    ...
    public function updatecharityProfile(CharityFrontFormRequest $request)
    {
        $charity = Charity::findOrFail(Auth::guard('charity')->user()->id);
        /*         * **************************************** */
	   if ($request->hasFile('logo')) {
            $is_deleted = $this->deleteCompanyLogo($charity->id);
            $image = $request->file('logo');
            $fileName = ImgUploader::UploadImage('company_logos', $image, $request->input('organization_name'), 300, 300, false);
            $charity->logo = $fileName;
        }
        /*         * ************************************** */

        $charity->organization_name = $request->input('organization_name');
        $charity->email = $request->input('email');
        if (!empty($request->input('password'))) {
            $charity->password = Hash::make($request->input('password'));
        }
        
        $charity->industry_id = $request->input('industry_id');
        $charity->location = $request->input('location');
        $charity->phone_number = $request->input('phone_number');
        $charity->short_description = $request->input('short_description');
        $charity->description = $request->input('description');
        $charity->contact_name = $request->input('contact_name');
        $website = $request->input('website');
        $charity->website = (false === strpos($website, 'http')) ? 'http://' . $website : $website;
        $charity->job_title = $request->input('job_title');
		$charity->slug = str_slug($charity->organization_name, '-').'-'.$charity->id;
        $charity->update();
	...
	public function sendContactForm(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            $receiver_company = Charity::findOrFail($request->input('to_id'));
    ...
    {
		$jobs = Auth::guard('charity')->user()->jobs()->paginate(10);
    ...
    public function donorProfile($id)
    {
		...
		$profileCv = $donor->getDefaultCv();
		
		/*************************************************/
		$num_profile_views = $donor->num_profile_views + 1;
		$donor->num_profile_views = $num_profile_views;
		$donor->update();
		/*************************************************/

		return view('charity.donor_profile')
            ->with('donor', $donor)
            ->with('profileCv', $profileCv)
            ->with('page_title', 'Job Seeker Profile')
            ->with('form_title', 'Contact Job Seeker');
	...
	public function charityFollowers()
    {
        $company = Charity::findOrFail(Auth::guard('charity')->user()->id);
	...
	public function charityMessages()
    {
        $company = Charity::findOrFail(Auth::guard('charity')->user()->id);
    ...
    {
        $company = Charity::findOrFail(Auth::guard('company')->user()->id);
    ...
    public function findDonors(Request $request)
    {
        ...
        $limit = 10;
        $donors = $this->fetchDonors($search, $limit);
        return view('charity.donor_list')
                    ->with('donors', $donors);
    }
