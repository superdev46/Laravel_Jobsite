...
class Job extends Model
{
    ...
    public function getDonor($field = '')
    {
        if(null !== $donor = $this->donor()->first()){
			if (!empty($field)) {
				return $donor->$field;
			} else {
				return $donor;
	...
	public function getJobSkillsList()
    {
		...
		if($this->jobSkills->count()){
			...
			foreach($jobSkills as $jobSkillManager){
				...
				$str .= '<span><a href="'.route('job.list', ['job_skill_id[]'=>$skill->job_skill_id]).'">'.$skill->job_skill.'</a></span>';
			}
		}
		return $str;
    }
    ...
    public function getAppliedCharityIdsArray()
    {
        return $this->jobApplies->pluck('charity_id')->toArray();
    }
	/********************************/
