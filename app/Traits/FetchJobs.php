...
trait FetchJobs
{
    ...
    public function fetchJobs($search = '', $job_titles = array(), $donor_ids = array(), $industry_ids = array(), $job_skill_ids = array(),$functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $job_type_ids = array(), $job_shift_ids = array(), $salary_from = 0, $salary_to = 0,$is_featured = -1, $orderBy = 'id', $limit = 10)
    {
        $asc_desc = 'DESC';

        $query = Job::select($this->fields);
        $query = $this->createQuery($query, $search, $job_titles, $donor_ids, $industry_ids, $job_skill_ids,$functional_area_ids, $country_ids, $state_ids, $city_ids, $job_type_ids, $job_shift_ids, $salary_from, $salary_to, $is_featured);
	...
	public function fetchIdsArray($search = '', $job_titles = array(), $donor_ids = array(), $industry_ids = array(), $job_skill_ids = array(),$functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $job_type_ids = array(), $job_shift_ids = array(),    $salary_from = 0, $salary_to = 0,  $is_featured = -1, $field = 'jobs.id')
    {
	...
	public function createQuery($query, $search = '', $job_titles = array(), $donor_ids = array(), $industry_ids = array(), $job_skill_ids = array(),$functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $job_type_ids = array(), $job_shift_ids = array(),    $salary_from = 0, $salary_to = 0,  $is_featured = -1)
	{
        ...
        if (isset($company_ids[0])) {
            $query->whereIn('jobs.donor_id', $donor_ids);
        }
		if (isset($industry_ids[0])) {
			$company_ids_array = User::whereIn('industry_id',$industry_ids)->pluck('id')->toArray();
            $query->whereIn('jobs.donor_id', $company_ids_array);
        }
	...
	private function getSEO($functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $career_level_ids = array(), $job_type_ids = array(), $job_shift_ids = array(),   $job_experience_ids = array())
	{
		$description = 'Jobs ';
		$keywords = '';
		if(isset($functional_area_ids[0])){
